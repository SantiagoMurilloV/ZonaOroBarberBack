...
import { faHome, faStore, faCalendarAlt, faClock, faTrash, faBook, faHistory, faChartColumn, faPlus } from '@fortawesome/free-solid-svg-icons';
...
const FinancesV2 = () => {
  ...
  const fetchImageUrl = async () => {
    try {
      const watermark = ref(storage, 'waterMark.png');
      const urlWaterMark = await getDownloadURL(watermark)
      setIcon(urlWaterMark)
    } catch (error) {
  ...
  const handlePlanSelect = (selectedOption) => {
    ...
    let endDate;
    if (selectedOption.value === 'Mensual') {
  ...
  return (
    <div>
      ...
      <div className="filters-container">
        ...
        <button id="refreshButton" className='butom-day-finance-hist'>Actualizar </button>
        <Link to={`/store/${id}`}>
        ...
        <img src={icon} alt="" className="watermark-history" />
      </div>
      {loading ? (
            ...
            <form onSubmit={handleFormSubmit}>
              <label>
                ...
                <Select
                  ...
                  options={uniqueUsers.map(user => ({ value: user.userId, label: `${user.FirstName} ${user.LastName}` }))}
                />
              </label>
              <label>
                Tipo de Plan:
