...
import { faArrowLeft, faArrowRight, faCalendarDay, faCalendarPlus, faHome, faUserFriends, faCalendarMinus, faDollarSign, faBarChart, faBarsProgress, faChartBar } from '@fortawesome/free-solid-svg-icons';
...
const Diary = () => {
  ...
  const handleDeleteUserReservations = async () => {
    ...
    try {
      const response = await fetch(`${environment.apiURL}/api/reservations?userId=${selectedUserId}&startDate=${formattedDeletionDate}`);
      if (!response.ok) throw new Error('Error al recuperar reservas');

      const reservations = await response.json();
      const reservationsToDelete = reservations.filter(reservation =>
        moment(reservation.day).isSameOrAfter(formattedDeletionDate) && reservation.userId === selectedUserId);

      const totalReservations = reservationsToDelete.length;
      ...
      for (const reservation of reservationsToDelete) {
        ...
        completedDeletions++;
        setDeletionProgress((completedDeletions / totalReservations) * 100);
      }
          ...
          const reservationCells = spaceAvailable.map((available, index) => {
            ...
            if (available) {
              if (reservation) {
                ...
                const userFullName = fullName.length > 12 ? fullName.slice(0, 20) + '..' : fullName;
                return (
                  <div key={reservation._id} className="reservation-cell bordered-cell">
                    ...
                    <div className="subcolumn training-type">
                      ...
                      >
                        <option value=' '> </option>
                        <option value='Tren Superior'>Sup</option>
                        <option value='Jalon'>Jal</option>
                        <option value='Empuje'>Emp</option>
                        <option value='Brazo'>Brz</option>
                        <option value='Pierna'>Prna</option>
                        <option value='Gluteo'>Glt</option>
                        <option value='Cardio'>Crd</option>
                        <option value='Primer dia'>FB</option>
                      </select>
                    </div>
  ...
  return (
    <div className={`Diary-container ${loading ? 'fade-in' : 'fade-out'}`}>
      ...
      <div className="filter-controls-diary">
        ...
        <button className='butom-day-diary-avilable' onClick={() => setShowDeleteModal(true)}>
          Eliminar
        </button>
        <button id="refreshButton" className='butom-day-diary-avilable'>Actualizar </button>
        <Link to={`/admin/${id}`}>
          <button className='butom-day-diary' >
      ...
      {showDeleteModal && (
        <div className="overlay">
          <div className="reservation-form">
            ...
            <input
              type="date"
              value={deletionDate ? new Date(deletionDate).toISOString().split('T')[0] : ''}
              onChange={(e) => handleDeletionDateChange(new Date(e.target.value))}
            />
      ...
      {showDeletionProgress && (
        <div className="progress-container">
          <div className="progress-bar" style={{ width: `${deletionProgress}%` }}></div>
        </div>
      )}
